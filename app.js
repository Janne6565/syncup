let vue = new Vue({
  ...
  data: {
    ...
    lineups: [
      {
        ...
        map: {
          name: "Loading...",
          image:
            "https://projektejwkk.de/valorantLineups/assets/mapIconsScaledDown/Ascent.webp",
        },
        ability: {
    ...
    userLineups: [],
    isUploading: false,
    extendedUserLineup: null,
    navigationOptions: {
      top: [
        {
          title: "Home",
          icon: "fa-solid fa-home fa-2x redirectButton redirecthome",
          link: "home",
          class: "icon",
          tooltip: "Home",
        },
        {
          title: "Maps",
          icon: "fa-solid fa-map-location-dot fa-2x redirectButton redirectmaps",
          link: "maps",
          class: "icon",
          tooltip: "Maps",
        },
        {
          title: "Ingame",
          icon: "fa-solid fa-gamepad fa-2x redirectButton redirectingame",
          link: "ingame",
          class: "icon",
          tooltip: "Ingame",
        },
        {
          title: "Upload",
          icon: "fa-solid fa-upload fa-2x redirectButton redirectupload",
          link: "upload",
          class: "icon",
          tooltip: "Upload",
        },
      ...
      bottom: [
        {
          title: "Settings",
          icon: "fa-solid fa-cog fa-2x redirectButton redirectsettings",
          link: "settings",
          class: "botIcon settingsIcon",
          tooltip: "Settings",
        },
        {
          title: "User",
          icon: "fa-solid fa-user fa-2x redirectButton redirectuser",
          link: "user",
          class: "botIcon userIcon",
          tooltip: "User",
        },
      ],
    },
  ...
  methods: {
    ...
    loadRemote: function () {
      ...
      generateRemoteKey(userId, authKey, (status, key) => {
        console.log(status, key);
      });
    },
    extendUserLineup: function (id) {
      let disextendLineup = (lineupId) => {
        let lineup = document.getElementById("userLineup" + lineupId);
        lineup.classList.remove("extended");
      };

      let extendLineup = (lineupId) => {
        let lineup = document.getElementById("userLineup" + lineupId);
        lineup.classList.add("extended");
      };

      if (this.extendedUserLineup == id) {
        disextendLineup(id);
        this.extendedUserLineup = null;
      } else {
        if (this.extendedUserLineup != null) {
          disextendLineup(this.extendedUserLineup);
        }
        extendLineup(id);
        this.extendedUserLineup = id;
      }
    },
    loadUserLineups: function () {
      ...
      getUserLineups(userId, authKey, (lineups) => {
        ...
        for (let lineup of self.userLineups) {
          lineup["date"] = getTimeStringFromTimestamp(lineup["DateCreated"]);
        }
      });
    },
    ...
    setPage: function (page) {
      ...
      document.title = "SyncUp - " + namesOfPages[page];

      let menu = document.getElementById("menuToggle");
      menu.classList.remove("active");

      if (this.activePage == page && page != "home") {
        this.setPage("home");
        return;
      }
      ...
      getLineups(
        ...
        (status, lineups, maxCount) => {
          if (status == 200) {
            ...
            let maxCount = isMobileDevice() ? 4 : 15;
            ...
            for (const lineup of lineups) {
              if (count <= maxCount) {
                ...
                newLineups.push(lineupCorrect);
              }

              count += 1;
            }
            self.lineups = newLineups;
    ...
    registerSwipeListener: function () {
    ...
    isScrollableHorizontally(element) {
      return (
    ...
    handleTouchMove(evt) {
      ...
      if (
        (!this.isScrollableHorizontally(this.targetElement) &&
        !this.isScrollableHorizontallyOrAncestor(this.targetElement)) || this.isMenuOpen()
      ) {
    ...
    isScrollableHorizontallyOrAncestor(element) {
      if (!element) {
        return false;
      }
      return (
        this.isScrollableHorizontally(element) ||
        this.isScrollableHorizontallyOrAncestor(element.parentElement)
      );
    },
  },
  mounted: function () {},

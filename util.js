...
API_URL =
  "https://" +
  API_URL.replace("/login/", "")
    .replace("/register/", "")
    .replace("/forgotPassword/", "")
    .replace("//", "/");

function isMobileDevice() {
  const mobileScreenWidthThreshold = 1000; // in pixels
...
function generateRemoteKey(userId, authKey, recall) {
  let xml = new XMLHttpRequest();

  const url = echoParams(API_URL + "action/generateRemoteKey.php", {
    userId: userId,
    authKey: authKey,
  });
